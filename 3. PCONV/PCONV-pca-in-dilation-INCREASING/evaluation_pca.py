...
def evaluate(model, dataset, filename, GPU_YN_IN=False):
    ...
    if GPU_YN_IN==True:
        grid = make_grid(
        # torch.cat((unnormalize(image), mask.cpu(), unnormalize(output),
        #            unnormalize(output_comp), unnormalize(gt),full_mask,full_mask_2), dim=0))
        # torch.cat((image, mask, output,
                   # output_comp, gt,full_mask), dim=0))
        torch.cat((gt_concat, output_concat,gt_mask_concat ), dim=0)) #image_concat  --do we need this?

        # torch.cat((pca, gt_mask, output,
        ...
        grid = make_grid(
            # torch.cat((unnormalize(image), mask.cpu(), unnormalize(output),
            #            unnormalize(output_comp), unnormalize(gt),full_mask,full_mask_2), dim=0))
            # torch.cat((image, mask.cpu(), output,
                       # output_comp, gt,full_mask), dim=0))
