...
crp_im_PATH = project_path+'cropped_imgs/' 
...
def random_box_inbb(img, size=30, display_yn=False, perc_zero_cutoff=0.2):
    ...
    try:
        ...
        if display_yn == True:
            ...
            print('\n'+'*'*40+'\nimg_copy2')
            display(img_mask)
        return img_copy, img_mask, error_score, cropped_im, upper_rnd, left_rnd
    except (ValueError,TypeError) as e:
        # Show the black image
        return img, img, 1, cropped_im
...
class UVDataset(Dataset):
    ...
    def __init__(self, csv_path, img_path, box_path, mask_path, crp_path, coords_u, coords_l, transform=None, train=True):
    ...
    def __getitem__(self, index):
        ...
        return img, label, img_box, img_mask, crop, coord_box_u, coord_box_l
...
def data_gen(norm, coords_u, coords_l):
    norm = norm
    # New Box + Mask
    for i in range(1):
        if norm == 'Y':
            train_set = UVDataset(csv,IMG_PATH, box_im_PATH, box_mask_PATH, crp_im_PATH, coords_u, coords_l, transform=transform1, train=True)
            test_set = UVDataset(csv, IMG_PATH, box_im_PATH, box_mask_PATH, crp_im_PATH, coords_u, coords_l, transform=transform1, train=False)
            print('Normalised')
        # Original
        else:
            train_set = UVDataset(csv,IMG_PATH, box_im_PATH, box_mask_PATH, crp_im_PATH, transform=transform2, train=True)
            test_set = UVDataset(csv, IMG_PATH, box_im_PATH, box_mask_PATH, crp_im_PATH, transform=transform2, train=False)

            train_set = UVDataset(csv,IMG_PATH, box_im_PATH, box_mask_PATH, crp_im_PATH, transform=transform1, train=True)
            test_set = UVDataset(csv,IMG_PATH, box_im_PATH, box_mask_PATH, crp_im_PATH, transform=transform1, train=False)
