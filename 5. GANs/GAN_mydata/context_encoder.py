...
def save_sample(batches_done, test_dataloader):
    ...
    for i in range(0,len(imgs)):
        ...
        try:
            test_plot_imgs
            # print('Append')
            test_plot_imgs = torch.cat((test_plot_imgs, filled_samples))
        except:
            # print('Created first')
            test_plot_imgs = filled_samples

    test_loss = pixelwise_loss(gen_mask, masked_parts)

    # Save sample
    sample = torch.cat((masked_imgs.data, test_plot_imgs.data, imgs.data), -2)
...
for epoch in range(n_epochs):
    coord_u, coord_l = p.remask('Y', df=df_train)
    train, test = p.data_gen(norm='Y', coords_u=coord_u, coords_l=coord_l)
